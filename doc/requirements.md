...
3. **全ルート網羅テスト & デグレ（回帰）チェックの必須実施**:
   - **全ルート確認**: Webポータルの各ページ（一覧、詳細、最近、年表）、API（check, download）、フィード（RSS, XML）、リリース同期・ZIP生成、ログ記録など、全実行ルート・エンドポイントを網羅して検証。
...
### 4.8 セキュリティ & 運用管理方針（リードオンリー型Webポータル）
...
2. **高い堅牢性とゼロアタックサーフェス**:
   ...
   - Webポータルは「閲覧・ダウンロード・手動同期実行・API配信」に特化した堅牢なシステムとする。

### 4.9 非機能要件・パフォーマンス & 異常系フォールバック仕様
